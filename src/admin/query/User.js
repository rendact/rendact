...
const setAsOwner = function(newOwner, allUserId, accessLevel) {
  ...
  _.forEach(allUserId, function(val, index) {
    query +=
      " removeFromUserRoles" +
      index +
      ' : removeFromUserRolesConnection(input: {userId: "' +
      val +
      '", roleId: "Um9sZToy"}){ changedUserRoles{ user {id} } }';
  });
  query +=
    ' addToUserRoles1 : addToUserRolesConnection(input: {userId: "' +
    newOwner +
    '", roleId: "Um9sZToy", accessLevel: admin}){ changedUserRoles{ user {id} } }';
  query += "}";
...
const checkUsernameQry = function(username) {
  return {
    query:
      'query checkUsername{ viewer { allUsers(where: {username: {like: "' +
      username +
      '"}}) { edges { node { id } } } } }'
  };
};
...
const checkEmailQry = function(email) {
  return {
    query:
      'query checkEmail{ viewer { allUsers(where: {email: {like: "' +
      email +
      '"}}) { edges { node { id } } } } }'
  };
};
