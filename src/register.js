...
const Register = React.createClass({
    ...
    disableForm: function(state) {
        var id = ["username", "fullname", "email", "password", "repassword", "registerBtn"];
        for (var i = 0; i < id.length; i++) {
            document.getElementById(id[i]).attributes.disabled = state;
        }
        this.setState({
            loadingMsg: state ? "Sending..." : null
        });
    },
    ...
    handleSubmit: function(event) {
        ...
        var id = ["fullname", "gender", "username", "email", "password", "repassword"],
            val = [];
        for (var i = 0; i < id.length; i++) {
            val[id[i]] = document.getElementById(id[i]).value;
        }
        ...
        request({
            ...
            body: Query.createUserMtn(val["username"], val["password"], val["email"], val["fullname"], val["gender"])
        }, (error, response, body) => {
            if (!error && !body.errors && response.statusCode === 200) {
                ...
                sendMail(val["email"], "Welcome to Rendact", val["fullname"] + "! Welcome to Rendact");
